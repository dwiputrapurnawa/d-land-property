...
<div id="whatsappModal" class="fixed inset-0 flex items-center justify-center z-50 hidden bg-black bg-opacity-40">
    <div class="relative flex flex-col text-white w-[80%] md:w-[37%] mx-auto rounded-lg overflow-hidden bg-black md:bg-transparent">
        ...
        <header class="flex flex-col items-center px-6 sm:px-10 md:px-20 pt-8 sm:pt-10 md:pt-14 pb-6 sm:pb-8 w-full shadow-2xl bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg">
            <div class="flex flex-col max-w-full w-full sm:w-[300px]" itemscope itemtype="http://schema.org/Organization">
                ...
                <div class="p-10">
                  <img src="data:image/png;base64, {!! base64_encode(QrCode::format('png')->size(256)->backgroundColor(0, 0, 0, 0)->color(255, 255, 255)->generate('https://wa.me/' . str_replace([' ', '-'], '', $company->phone))) !!} " class="object-contain mt-8 sm:mt-12 md:mt-16 w-full max-w-[200px] sm:max-w-full mx-auto aspect-square lazy" alt="wa-img">
                </div>
                <a href="https://wa.me/{{ str_replace([' ', '-'], '', $company->phone) }}" aria-label="Send us a message on Whatsapp" class="flex items-center justify-center gap-3 sm:gap-5 px-5 sm:px-7 py-3 sm:py-3.5 mt-8 sm:mt-12 md:mt-16 text-sm sm:text-base tracking-wide border border-white border-solid font-medium rounded-full hover:bg-white hover:bg-opacity-10 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-50" itemprop="contactPoint" itemscope itemtype="http://schema.org/ContactPoint">
                  <img loading="lazy" src="{{ asset('images/whatsapp.webp') }}" alt="wa-text" class="object-contain shrink-0 w-5 sm:w-6 aspect-square lazy" />
